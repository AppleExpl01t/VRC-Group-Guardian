...
import src.services.mock_log_watcher
import src.services.log_watcher
# Inject mock logic
src.services.log_watcher.get_log_watcher = src.services.mock_log_watcher.get_log_watcher
...
import flet as ft
from src.api.mock_client import MockVRChatAPI as VRChatAPI # Demo Mode
# src.api.client NOT imported (simulating "live code excluded")

from src.ui.theme import setup_theme, colors, spacing
from src.ui.components.sidebar import Sidebar
from src.ui.components.animated_background import SimpleGradientBackground
from src.ui.views.login import LoginView
from src.ui.views.welcome import WelcomeView
from src.ui.views.group_selection import GroupSelectionView
from src.ui.views.dashboard import DashboardView
from src.ui.views.instances import InstancesView
from src.ui.views.requests import RequestsView
from src.ui.views.members import MembersView
from src.ui.views.bans import BansView
from src.ui.components.title_bar import TitleBar
from src.ui.views.settings import SettingsView
from src.ui.views.live_instance import LiveInstanceView
from src.services.log_watcher import get_log_watcher

# Need to import these for the group header
from src.ui.theme import radius, typography
...
class GroupGuardianApp:
    ...
    def __init__(self, page: ft.Page):
        ...
        # Show login immediately so UI is visible (and Demo Mode accessible)
        self._show_login()
        
        # Determine platform capabilities
        # Live Monitor now uses MOCK watcher, so it is always supported in demo
        self._supports_live = True
    ...
    async def _load_data_and_transition(self):
        ...
        # Start data loading (fire and forget)
        self.page.run_task(self._load_groups)
        
        # Strict wait for cinematic feel
        await asyncio.sleep(3.5)
        
        # Always transition to group selection after the welcome animation
    ...
    def _show_group_selection(self):
        ...
        # Fetch groups async (only if needed)
        self.page.run_task(self._load_groups)
    ...
    def _build_main_view(self, route: str) -> ft.View:
        ...
        if route == "/live":
            ...
            # But the sidebar handles routes. We can add a "Back" item? 
            # Or reliance on the "Switch Group" / "Back to Groups" button in header is enough?
            # User request: "without the instances requests members bans or logs tabs".
            # It implies removing them. Keeping "Live".
    ...
    def _get_content_for_route(self, route: str) -> ft.Control:
        ...
        if route == "/dashboard":
            ...
            return SettingsView(on_navigate=self._navigate)
